...
//Create a string represent the save file of the wallet.
    string file = "Project/Saved Wallet/";
    file += id;
    file += ".bin";
    Transaction t;
    std::ifstream fin;
...
void Wallist :: outSC() {
    std::ofstream fout;
    fout.open("Project/Utility Save Files/Income.bin", std::ios::binary);
    if (!fout.is_open()) {
    ...
    fout.close();
    fout.open("Project/Utility Save Files/Expense.bin", std::ios::binary);
    if (!fout.is_open()) {
...
void Wallist :: outWallist() {
    std::ofstream file;
    file.open("Project/Utility Save Files/WalletNumber.bin", std::ios::binary);
